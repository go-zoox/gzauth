...
type Config struct {
	...
	ClientID string
	// ClientSecret is the github client secret
	ClientSecret string
	// RedirectURI is the github redirect uri
	RedirectURI string
...
func Serve(cfg *Config) error {
	...
	app.Use(func(ctx *zoox.Context) {
		...
		// login
		if ctx.Path == "/login" || ctx.Path == "/login/github" {
		...
		// callback
		if ctx.Path == "/login/callback" || ctx.Path == "/login/github/callback" {
